...
class TiuRuVendorModel extends YandexMarketVendorModel {
   ...
   public function updateCategories($intProfileID) {
      ...
      if (strlen($strFileContent)) {
         ...
         $strTmpFile = $strTmpDir . '/categories.' . pathinfo(static::CATEGORIES_XLS_URL, PATHINFO_BASENAME);
         if (file_put_contents($strTmpFile, $strFileContent)) {

            Helper::includePhpSpreadSheet();
            $strExcelType = \PhpOffice\PhpSpreadsheet\IOFactory::identify($strTmpFile);
            $reader = IOFactory::createReader($strExcelType);
            $reader->setReadDataOnly(true);

            $obExcel = $reader->load($strTmpFile);

            $sheetData = $obExcel->getActiveSheet()->toArray(null, true, true, true);

            $strCategories = '';
            foreach ($sheetData as $cell) {
               if ($cell['A'] == 'Категория1')
                  continue;
               $strCategories .= $cell['F'] . ' - ' . $cell['A'];
               if ($cell['B'])
                  $strCategories .= ' / ' . $cell['B'];
               if ($cell['C'])
                  $strCategories .= ' / ' . $cell['C'];
               if ($cell['D'])
                  $strCategories .= ' / ' . $cell['D'];
