...
class AliexpressComYml extends AliexpressCom {
	...
	/**
	 *	Correct item categoryId
	 *	(If product linked to two sections, one of which is selected for export, and second is not selected for export,
	 *	this handler correct wrong section to right).
	 */
	protected function onUpBeforeProcessElement(&$arResult, &$arElement, &$arFields, &$arElementSections, $intMainIBlockId, &$bStop){
		if(is_array($arElementSections) && !empty($arElementSections) && !in_array($arFields['categoryId'], $arElementSections)){
			$arFields['categoryId'] = reset($arElementSections);
		}
	}
