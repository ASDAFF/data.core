...
<?if(!empty($arCurrentTasksMatch)):?>
	<div>
		<a href="javascript:void(0)" data-role="cron-current-tasks-toggle" class="data-inline-link">
			<?=Loc::getMessage('DATA_EXP_CRON_TASKS_TOGGLE');?>
		</a>
		(<?=count($arCurrentTasksMatch);?>)
		<div data-role="cron-current-tasks" style="display:none;">
			<?foreach($arCurrentTasksMatch as $arTask):?>
				<div class="data-exp-cron-form-command" style="margin-top:4px;">
		...
		<?if(count($arCurrentTasksMatch) > 1):?>
			<div style="margin-top:8px;">
				<?=Helper::showNote(Loc::getMessage('DATA_EXP_CRON_TASKS_MULTIPLE_NOTICE'), true);?>
			</div>
		<?endif?>
	</div>
<?else:?>
	<?=Loc::getMessage('DATA_EXP_CRON_TASKS_NO');?>
<?endif?>
